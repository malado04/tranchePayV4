                    ...
                    <table id="example" class="table table-striped" style="width:100%">
                        <thead>
                            <tr>
                                ...
                                <th scope="col">Total des versement</th>
                                <th scope="col">Montant restant</th>
                                <th scope="col">Verset</th>
                            </tr>
                        </thead>
                        <tbody>
                            @foreach ($listecommandesclient as $commandes)
                            ...
                            @if(Auth::user()->id == $commandes->client_id)
                            <tr>
                                <th>{{$commandes->user->boutique}}</th>
                                <th>{{$commandes->id}}</th>
                                <th>{{$commandes->delaipaye}}</th>
                                <th>{{$commandes->created_at}}</th>
                                <th>{{$commandes->montantpayer}}</th>
                                @php
                                    $mtvcommande=$commandes->montantverset;
                                @endphp
                                @foreach ($listeversement as $versement)
                                @if(Auth::user()->id == $versement->user_id && $commandes->id == $versement->enregistrercommande_id)
                                    @php
                                        $mtvcommande=$mtvcommande+$versement->verset;
                                    @endphp
                                @endif
                                @endforeach
                                <th>{{$mtvcommande}}</th>
                                @php
                                    $mrestant=$commandes->montantpayer -$mtvcommande;
                                @endphp
                                <th>{{$mrestant}}</th>
                                @if($mrestant > 0) 
                                <th>
                                    <a class="btn btn-warning " style="width: 85px;" href="{{route('effectuerversement',['enregistrercommande'=>$commandes->id])}}" > 
                                        Verset 
                                    <a>
                                    <!-- <a class="btn btn-warning " style="width: 85px;" href="invoice" > 
                                        Verset 
                                    <a> -->
                                </th>
                                @elseif($mrestant <=0)
